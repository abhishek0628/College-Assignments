//problem
/*
1. Write a program to create a Special Binary Search Tree T with distinct keys (with values in the
range [1, 106]) and perform related operations. A Special BST is organized as a binary tree in which
each node is an object that contains a key value. In addition to a key, each node contains three
attributes left, right, and parent that point to the nodes corresponding to its left child, right child,
and parent, respectively. If a child or the parent is missing, the appropriate attribute contains the
value NIL. The root node is the only node in the tree whose parent is NIL. The keys in a special
binary search tree are always stored in such a way as to satisfy the following property:
...
• For option ‘s’, if the key is present in T, then print ’F’. If the key is not present in T, then
print ’N’. Print NIL if T is empty.
